...
function convertLatexToOmml(latex: string): string {
  ...
  result = result.replace(/(\w)\^{([^}]+)}/g, (_, base, sup) => {
    const placeholder = `§PLACEHOLDER§${placeholders.length}§`;
    placeholders.push(`<m:sSup><m:e><m:r><m:t>${base}</m:t></m:r></m:e><m:sup><m:r><m:t>${sup}</m:t></m:r></m:sup></m:sSup>`);
  ...
  result = result.replace(/(\w)_{([^}]+)}/g, (_, base, sub) => {
    const placeholder = `§PLACEHOLDER§${placeholders.length}§`;
    placeholders.push(`<m:sSub><m:e><m:r><m:t>${base}</m:t></m:r></m:e><m:sub><m:r><m:t>${sub}</m:t></m:r></m:sub></m:sSub>`);
    return placeholder;
  });
